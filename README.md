...
| `src/ingest_kalshi.py` | Ingestion (backend) · MVP data adapter | `KalshiSource`: `MarketDataSource` over Kalshi's trade-api v2 (weather + econ, ADR-0006, WP-3). Public/unauthenticated by default; pass `credentials=KalshiCredentials.from_env()` for RSA-PSS-signed authenticated access (ADR-0018) — every endpoint used here is reachable either way. `wallet_trades`/`leaderboard` raise `NotImplementedError` (Kalshi has no public per-trader feed). Data only — no order placement. |
...
| `src/fees.py` | Fee math | Polymarket V2 taker formula `rate·p·(1−p)` (maker-free) + Kalshi's per-order rounded fee: taker `0.07`, maker `0.0175` on maker-fee-enabled series and free elsewhere, `0.035` for S&P500/NASDAQ-100 — all verified against the primary fee schedule and pinned cell-by-cell by `tests/test_fees.py` (ADR-0015). The single source of truth every other module imports. |
